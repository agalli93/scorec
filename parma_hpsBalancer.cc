...
namespace parma {
  ...
  class MergeTargets : public Targets {  // we don't really need a map/associative container here - a list/vector/array would work
    public:
      ...
      {
		w.begin() // Initialize the iterator for weights
		//Look into using while loop
		for( Item weight = /*itr begin for weights*/ ; itr != NULL; itr = w.iterate())
		{
			if (weight.second < maxW && weight.second > 0){
				int* value = new int[s.get(weight.first)];
				std::fill (value, value + s.get(weight.first),1); //should this be value + s.get(weight.first)-1
				const double minWeight = //...
				int* normalizedIntWeights = new int[s.get(weight.first)];
				for(int weightIdx = 0; weightIdx < s.get(weight.first); weightIdx++){
					normalizedIntWeights[weightIdx] = (int) ceil( s.get(weight.first)[weightIdx] / minWeight );
				}
				const double weightCapacity = maxW - weight.second;
				const int knapsackCapacity = floor(weightCapacity/minWeight);
				const float imbalance = (float) weight.second / (float) avgWeight(w);
				
				sprintf(dbgMsg, "[%d] (%d) %s "
							"weight= %lf  "
							"imbalance = %.2f "
							"avgWeight*maxImb = %.2f "
							"maxImb = %.2f "
							"minWeight = %lf  "
							"capacity = %lf  "
							"capacity/minWeight = %d\n",
							rank, weight.first, __FUNCTION__,
							weight.second,
							imbalance,
							maxW,
							maxW / avgWeight(w),
